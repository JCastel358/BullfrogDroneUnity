...
{
    [Serializable]
    public class RemainingTrajectoryGizmos : BaseTrajectoryLinesGizmos
    {
        [Tooltip("Specifies how the trajectory should be considered 'ended'. FixedFloor uses the last fixed update, " +
                 "FixedCeiling extends slightly past it, and Absolute uses the full duration.")]
        public RemainingEndTypes EndType = RemainingEndTypes.FixedFloor;
        ...
        public void DrawGizmos(Trajectory trajectory, Trajectory relativeTo = default)
        {
            ...
            relativeTo.GetRemainingTrajectoryTimes(EndType, out var tFrom, out var tTo);
            switch (Type)
            {
